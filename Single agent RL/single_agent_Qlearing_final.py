# -*- coding: utf-8 -*-
"""
Created on Sat Sep 28 13:31:43 2019
...
#import sys
#origin = sys.stdout
#f = open('single_qlearn_train_log.txt', 'w')
#sys.stdout = f
## ===================================
...
#        enhance_list = []
    ...
    def update(self):
        ...
        """
        self.RL = QLearningTable(actions=list(range(self.n_actions)),e_greedy=self.epsilon)
        episode = 0
        action = -1
        stepCount = 0       
        total_reward_list = []
            ...
            if done:
                if episode > self.episode:
                    ...
                    if len(total_reward_list) > 100:
                        ...
                        print(template.format(episode, self.episode, stepCount, sum(win_history)/len(win_history), total_reward, avg_reward, t))
                    else:
                        template = "Episode: {:03d}/{:d} | StepCount: {:d} | Win rate: {:.3f} | Total rewards: {:.3f} | time: {}"
                        print(template.format(episode, self.episode, stepCount, sum(win_history)/len(win_history), total_reward, t))
                    episode+=1
                    stepCount = 0
                    total_reward = 0
                    visited = set()                    
...
#            load Q table
            terminal_str = str(self.selected_targets_position)+str(self.selected_Obstacles_position)+'episode3000'
            self.q_table = pd.read_csv("table terminal%s.csv"%terminal_str,index_col=0)            
