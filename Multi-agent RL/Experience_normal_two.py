...
#        env_size = 4   # envstate 1d size (1st element of episode)
        ...
        for i, j in enumerate(np.random.choice(range(mem_size), data_size, replace=False)):
            ...
            envstate_B, action_B, reward_B, envstate_next_B, game_over_B = self.memory[j]
            
            envstate = np.hstack((envstate_A,envstate_B))
...
#            print("Q_sa_total==============",Q_sa_total)
            ...
            targets[i] = np.hstack((targetsA,targetsB))
#            print("@@@@In get_data: targets[i] = "+str(targets[i]))              
    ...
    def choose_action(self, observation ,agentIndex):
        ...
        if np.random.uniform() < self.epsilon:
            ...
            if agentIndex == 1:
                action = np.argmax(self.predict(observation)[4:])
        else:
